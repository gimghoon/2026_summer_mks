...
function preservesExplicitIntent(intent: string, text: string): boolean {
  ...
  switch (protectedIntentKind(intent)) {
    case "money": {
      ...
      const request = /(?:송금해|입금해|결제해|갚아)\s*줘(?!서)|(?:돈|금전|금액|송금|입금|빌려|결제|환불).{0,18}(?:보내\s*줘(?!서)|해\s*줄래\?|할\s*수\s*있어\?|부탁해)|(?:보내\s*줘(?!서)|해\s*줄래\?|할\s*수\s*있어\?|부탁해).{0,12}(?:돈|금전|금액|송금|입금|빌려|결제|환불)/u.test(text);
