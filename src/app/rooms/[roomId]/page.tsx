...
export default async function RoomPage({ params }: { params: Promise<{ roomId: string }> }) {
  ...
  return <main className="mobile-page"><header className="subpage-header"><Link href="/rooms" aria-label="대화방 목록으로">‹</Link><div><p className="eyebrow">대화방 검수</p><h1>{room.title}</h1></div></header><RoomAnalysisActions room={room} /><RoomParticipantList room={room} /><section className="danger-zone"><p className="eyebrow">삭제</p><h2>대화방을 지울까요?</h2><p>원문과 분석된 맥락, 프로필, 답장 기록이 함께 삭제돼요.</p><RoomDeleteButton roomId={room.id} /></section><BottomNav /></main>;
}
