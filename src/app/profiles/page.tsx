...
export default async function ProfilesPage() {
  await requireSession(); const rooms = await listRoomViews(); const people = rooms.filter((room) => room.analysisStatus === "ready").flatMap((room) => room.participants.filter((person) => !person.isSelf).map((person) => ({ ...person, room })));
