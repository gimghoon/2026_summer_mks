...
업로드는 현재 카카오톡의 `Date, User, Message` CSV 내보내기와 기존 한국어 날짜 기반 텍스트 내보내기를 모두 지원하며, 파일 크기 제한은 변함없이 50 MiB입니다.

## 검증된 네 화면 흐름

1. **대화방** — `/rooms`에서 카카오톡 `.txt` 또는 `.csv`를 업로드하고, 내 이름과 파싱되지 않은 줄을 검토한 뒤 분석을 시작합니다. 같은 방을 다시 가져오면 이미 저장된 메시지는 중복 저장하지 않습니다.
